...
function get_itens($post_type, $id = false)
{
    ...
    foreach ($editables as $editable) {
        ...
        if (isset($array[$type])) {
            @$array[$type][] = ['ID' => $editable->ID, 'title' => $editable->post_title, 'description' => $editable->post_content, 'itens' => $meta];
...
function create_types($type, $genre = 'male', $id, $slug, $singular, $plural, $support, $icon, $feature, $taxonomies, $status = 'publish', $comp = 'post')
{
...
function create_column($types, $name, $value, $sortable = false)
{
    add_filter('manage_' . $types . '_posts_columns', function ($columns) use ($value, $name) {
        $columns[$value] = "$name";
        return $columns;
    });

    add_action('manage_' . $types . '_posts_custom_column', function ($column_name, $post_id) use ($types, $value) {

        $string = 'get_post($post_id)->' . $value . ';';
        eval("\$string = $string;");

        if ($column_name == $value) {
            if ($value == 'editable_type') {

                $type = [
                    'conteudo_animes' => 'Conteúdo de Animes',
                    'seasons_animes' => 'Temporadas de Animes',
                    'highlights' => 'Destaques',
                    'Faq_card' => 'Carousel de Imagens',
                    'media' => 'Mídias',
                    'numbers' => 'Números',
                    'partners' => 'Parceiros',
                    'projects' => 'Projetos',
                ...
                $editable = get_post_meta($post_id, 'meta', true);
                echo $type[$editable['editable_type']];

            } elseif ($value == 'related') {
                $editable = get_post_meta($post_id, 'meta', true);
                echo get_post($editable['related'])->post_title;

            } elseif ($value == 'shortcode') {

                $editable = get_post_meta($post_id, 'meta', true);
                echo '<strong>[editable id="' . $post_id . '" type="' . $editable['editable_type'] . '"]</strong>';
    ...
    if ($sortable) {
        add_filter('manage_' . 'edit-' . $types . '_sortable_columns', function ($columns) use ($value) {
            $columns[$value] = $value;
            return $columns;
        });
    }
